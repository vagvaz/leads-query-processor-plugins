...
public class DefaultCrawler implements Crawler {
        ...
        try {
            ...
            if (page != null) {
                ...
                if (page.getResponseCode() == HttpURLConnection.HTTP_OK) {
                    log.debug(url + " was downloaded successfully. Download time "
                                  + page.getResponseTime());
                    Parser parser = parserController.getParser(page);
                    log.debug("Parsing " + url);
                    parser.parse(page);
                    crawlerTask.setTimeParsed(new Date());
                    log.debug(url + " has been parsed. " + page.getLinks().size()
                                  + " links were found");
                }
            }
        } finally {
    ...
    public boolean shouldCrawl(CrawlerTask crawlerTask, CrawlerTask parent) {
        return crawlerTask.getDomain() != null && parent.getDomain() != null && crawlerTask
                                                                                    .getDomain()
                                                                                    .equals(parent
                                                                                                .getDomain());
    }
