            ...
            if isarticle:
                ...
                if is_domain_verified is not None:
                    if is_domain_verified:
                        ...
                        analysis.process_domain(urlstr)
                        self.extr.extract_with_best_schema(urlstr, self.pagetype)


        elif vals[0] == 'comment':
            isarticle = self.verify.verify_article(vals[1])
            iscomments = self.verify.verify_comments(vals[2], tree)
