...
from eu.leads.infext.python.page_segmentation.terminal_colors import Tcolors
...
class SDAlgorithm():
    ...
    def analyze_page(self):
        ...
        tree = self.construct_page_tree_from_content()
        self.tree = tree
        node = tree.getroot()
        ...
        if article is not None and comments is None:
            ...
            return 'comment', article, comments, None
        else:
    ...
    def construct_page_tree(self):
        ...
        self.content = doc
        return self.construct_page_tree_from_content() 
    
    
    def construct_page_tree_from_content(self):
            ...
            if grouped_comments.has_key(max_group): 
                ...
                if self.big_areas_in_same_level(article, grouped_comments, max_group) and not validated:
                    ...
                    for com in grouped_comments[max_group]:
                        print element2path(com.tree,com.root_node), com.full_text              
...
if __name__ == '__main__': 
    url = "http://wiadomosci.onet.pl/swiat/kijow-i-moskwa-jak-wyjsc-z-powiazan-w-produkcji-broni/wzwmh"#sys.argv[1]
    sd = SDAlgorithm()
    sd.url = url
    sd.analyze_page()
